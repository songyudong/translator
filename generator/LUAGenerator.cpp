...
static void generateEnum(CodeFile& f, Enum* e)
{
    ...
    f.output("TranslatorTypes.%s = setmetatable(%s, TranslatorTypes._enum_mt)", name, name);
}
...
static void generateFieldDefaults(CodeFile& f, FieldContainer* fc)
{
    for (size_t i = 0; i < fc->fields_.size(); i++)
    {
        ...
        if (field.isArray())
            f.output("%s = TranslatorTypes.array(TranslatorTypes.%s),", name, type);
        else
            f.output("%s = TranslatorTypes.%s(),", name, type);
    }
}
static void generateFieldInitCode(CodeFile& f, FieldContainer* fc)
{
    for (size_t i = 0; i < fc->fields_.size(); i++)
    {
        ...
        f.indent();
        if (field.isArray())
            f.output("s.%s = TranslatorTypes.array(TranslatorTypes.%s, v.%s)", name, type, name);
        else
            f.output("s.%s = TranslatorTypes.%s(v.%s)", name, type, name);
        f.recover();
        f.output("end");
    }
}
static void generateFieldContainerSCode(CodeFile& f, FieldContainer* fc)
{
    for (size_t i = 0; i < fc->fields_.size(); i++)
    {
        Field& field = fc->fields_[i];
        if (field.isArray())
            f.output("TranslatorWriter.array(TranslatorWriter.%s, v.%s, b)", getFieldTypeName(field), field.getNameC());
        else
            f.output("TranslatorWriter.%s(v.%s, b)", getFieldTypeName(field), field.getNameC());
    }
}
static void generateFieldContainerDSCode(CodeFile& f, FieldContainer* fc)
{
    for (size_t i = 0; i < fc->fields_.size(); i++)
    {
        Field& field = fc->fields_[i];
        if (field.isArray())
            f.output("v.%s, p = TranslatorReader.array(b, p, TranslatorReader.%s, 0X%X, 0X%X)", field.getNameC(), getFieldTypeName(field), field.maxArray_, field.maxValue_);
        else
            f.output("v.%s, p = TranslatorReader.%s(b, p, 0X%X)", field.getNameC(), getFieldTypeName(field), field.maxValue_);
    }
}
...
static void generateStruct(CodeFile& f, Struct* s)
{
    ...
    f.output("TranslatorTypes.%s = function(v)", name);
    f.indent();
    ...
    f.output("setmetatable(s, TranslatorTypes._struct_mt)");
    ...
    f.output("TranslatorWriter.%s = function(v, b)", s->getNameC());
    f.indent();
    generateFieldContainerSCode(f, s);
    f.recover();
    f.output("end");

    f.output("TranslatorReader.%s = function(b, p)", s->getNameC());
    f.indent();
    f.output("local v = TranslatorTypes.%s()", s->getNameC());
...
void LUAGenerator::generate()
{
    ...
    CodeFile f(fn);
    f.output("require(\"translator.types\")");
    f.output("require(\"translator.reader\")");
    f.output("require(\"translator.writer\")");
