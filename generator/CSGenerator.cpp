...
static void generateEnum(CodeFile& f, Enum* e)
{
	...
	f.output("namespace translator");
	f.indent("{");
	f.output("public static partial class ProtocolReader");
	f.indent("{");
	f.output("public static bool read(translator.IReader r, ref %s v, uint maxValue)", e->getNameC());
	f.indent("{");
	...
	f.indent("{");
	f.output("public static void write(translator.IWriter w, %s v)", e->getNameC());
	f.indent("{");
...
static void generateFieldSCode(CodeFile& f, Field& field)
{
	...
	{
		f.indent("{");
		f.output("translator.ProtocolWriter.writeDynSize(__w__, (uint)%s.Count);", field.getNameC());
		f.output("foreach(%s __vi__ in %s) translator.ProtocolWriter.write(__w__, __vi__);",
			getFieldInnerTypeName(field),
			field.getNameC());
		f.recover("}");
	}
	else
		f.output("translator.ProtocolWriter.write(__w__, %s);", field.getNameC());
}
...
static void generateFieldDSCode(CodeFile& f, Field& field)
{
	if(field.isArray())
	{
		f.indent("{");
		f.output("uint __s__;");
		f.output("if (!translator.ProtocolReader.readDynSize(__r__, out __s__) || __s__ > 0X%X) return false;", field.maxArray_);
        ...
        f.output("if (!translator.ProtocolReader.read(__r__, ref __vi__, 0X%X)) return false;", field.maxValue_);
		f.output("%s.Add(__vi__);", field.getNameC());
		f.recover("}");
		f.recover("}");
	}
	else
		f.output("if(!translator.ProtocolReader.read(__r__, ref %s, 0X%X)) return false;", field.getNameC(), field.maxValue_);
}
...
static void generateStruct(CodeFile& f, Struct* s)
{
	...
	// serialize code.
	f.output("public void serialize(translator.IWriter __w__)");
	f.indent("{");
	generateFieldContainerSCode(f, s);
	f.recover("}");
	// deserialize code.
	f.output("public bool deserialize(translator.IReader __r__)");
	f.indent("{");
	...
	f.output("public static partial class ProtocolReader");
	f.indent("{");
	f.output("public static bool read(translator.IReader r, ref %s v, uint maxValue)", s->getNameC());
	f.indent("{");
	...
	f.output("public static partial class ProtocolWriter");
	f.indent("{");
	f.output("public static void write(translator.IWriter w, %s v)", s->getNameC());
	f.indent("{");
