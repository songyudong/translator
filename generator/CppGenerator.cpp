...
static const char* getFieldCppType(Field& f)
{
	...
	switch (f.type_)
	{
	case Field::FT_INT64:	name = "translator::INT64"; break;
	case Field::FT_UINT64: name = "translator::UINT64"; break;
	case Field::FT_DOUBLE: name = "translator::DOUBLE"; break;
	case Field::FT_FLOAT:	name = "translator::FLOAT"; break;
	case Field::FT_INT32:	name = "translator::INT32"; break;
	case Field::FT_UINT32: name = "translator::UINT32"; break;
	case Field::FT_INT16:	name = "translator::INT16"; break;
	case Field::FT_UINT16: name = "translator::UINT16"; break;
	case Field::FT_INT8:	name = "translator::INT8"; break;
	case Field::FT_UINT8:	name = "translator::UINT8"; break;
	case Field::FT_BOOL:	name = "translator::BOOL"; break;
	case Field::FT_STRING: name = "translator::STRING"; break;
	case Field::FT_BINARY: name = "translator::BINARY"; break;
	case Field::FT_ENUM:
...
static void generateStructDecl(CodeFile& f, Struct* s)
{
	...
	f.output("void serialize(translator::BinaryWriter* w) const;");
	f.output("bool deserialize(translator::BinaryReader* r);");
	f.recover("};");
}
...
static void generateFieldContainerSCode(CodeFile& f, FieldContainer* fc)
{
	for (size_t i = 0; i < fc->fields_.size(); i++)
	{
		...
		f.output("translator::ProtocolWriter::write%s(__w__, %s);",
...
static void generateFieldContainerDSCode(CodeFile& f, FieldContainer* fc)
{
	...
	{
		Field& field = fc->fields_[i];
		f.output("if(!translator::ProtocolReader::read%s(__r__, %s, 0X%X, 0X%X)) return false;",
			getFieldFuncName(field),
			field.getNameC(),
			field.maxArray_,
			field.maxValue_);
...
static void generateStructImp(CodeFile& f, Struct* s)
{
	...
	f.recover("}");
	f.output("void %s::serialize(translator::BinaryWriter* __w__) const", s->getNameC());
	f.indent("{");
	generateFieldContainerSCode(f, s);
	f.recover("}");
	f.output("bool %s::deserialize(translator::BinaryReader* __r__)", s->getNameC());
	f.indent("{");
...
static void generateHFile()
{
	...
	f.output("#include \"translator/ProtocolWriter.h\"");
	f.output("#include \"translator/ProtocolReader.h\"");
